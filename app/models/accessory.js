...
module.exports = {
    ...
    getMainItems: (slug, option = null) => { 
        let select = 'name slug brand category.name category.id price thumb content sale_off tags color reviews product_type';
        return Model.find({slug: slug}).select(select);
    },
    getAllItems: () => {
        return Model.find().select('name slug category.name price quantity sale_off brand thumb size color tags reviews product_type');
    },
